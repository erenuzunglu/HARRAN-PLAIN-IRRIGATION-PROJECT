# -*- coding: utf-8 -*-
"""
Created on Fri Aug  9 15:09:46 2024
...
#------ Buraya kadar olan kısım en büyük tarla için grafik elde edilmiş alan ------------
...
degree = 4  # You can change the degree to get a better fit
p_day_m = Polynomial.fit(day_data_m.index, day_data_m['Normalized Temperature'], degree)
p_day_all_m = Polynomial.fit(day_data_m.index, day_data_m['Mean Temperature'], degree)
p_night_m = Polynomial.fit(night_data_m.index, night_data_m['Mean Temperature'], degree)



# -------------- Buraya kadar olan kısım Tüm kesilmiş alan için bulunan sıcaklık ortalamalarının grafiği 


""

zip_file_path_ms = os.path.expanduser("~/Desktop/second_max_clipped_pixels.zip")
extracted_folder_path_ms = os.path.expanduser("~/Desktop/second_max_clipped_pixels/")
...
# Navigating into the directory to see the TIFF files
tiff_files_path_ms = os.path.join(extracted_folder_path_ms, 'second_max_clipped_pixels')
...
merged_data = pd.merge(day_data_ms, day_data, on='Date', suffixes=('_ms', ''))





#------------ buraya kadar 2 tarlanın ortalaması alındı, şimdi 3. taralyı da ekleyeceğiz-------------

zip_file_path_mt = os.path.expanduser("~/Desktop/third_max_clipped_pixels.zip")
extracted_folder_path_mt = os.path.expanduser("~/Desktop/third_max_clipped_pixels/")

# Extract the zip file
with zipfile.ZipFile(zip_file_path_mt, 'r') as zip_ref:
    zip_ref.extractall(extracted_folder_path_mt)

# List the files in the extracted folder
extracted_files_mt = os.listdir(extracted_folder_path_mt)
print(extracted_files_mt)

# Navigating into the directory to see the TIFF files
tiff_files_path_mt = os.path.join(extracted_folder_path_mt, 'third_max_clipped_pixels')
tiff_files_mt = os.listdir(tiff_files_path_mt)
print(tiff_files_mt)
...
# Process each TIFF file
for file_name_mt in sorted(tiff_files_mt):
    file_path_mt = os.path.join(tiff_files_path_mt, file_name_mt)
    with rasterio.open(file_path_mt) as src:
        array_mt = src.read(1)
        # Calculate the mean, ignoring nan values
        mean_value_mt = np.nanmean(array_mt)
        
        # Extract the date from the filename (YYYYMMDD)
        date_str_mt = file_name_mt.split('_')[2][:8]
        # Check if it is day or night
        day_or_night_mt = 'day' if 'day' in file_name_mt else 'night'
        
        data_mt.append((date_str_mt, day_or_night_mt, mean_value_mt))

# Convert the list into a DataFrame
df_mt = pd.DataFrame(data_mt, columns=['Date', 'Day/Night', 'Mean Temperature'])

# Convert the Date column to datetime format
df_mt['Date'] = pd.to_datetime(df_mt['Date'], format='%Y%m%d')

# Separate day and night data
day_data_mt = df_mt[(df_mt['Day/Night'] == 'day') & (df_mt['Mean Temperature'] > 290)]

scaler = MinMaxScaler()
day_data_mt['Normalized Temperature'] = scaler.fit_transform(day_data_mt[['Mean Temperature']])


night_data_mt = df_mt[(df_mt['Day/Night'] == 'night') & (df_mt['Mean Temperature'] > 277)]

#tüm dataları birleştiriyoruz

merged_data_final =pd.merge(day_data_mt, merged_data, on='Date')


# Aynı günlerin ortalamasını almak için, ilgili sütunları ortalama alarak yeni bir DataFrame oluşturun
average_data = pd.DataFrame()
average_data['Date'] = merged_data_final['Date']

# Örneğin, 'Mean Temperature' sütunlarının ortalamasını almak için:
average_data['Mean Temperature'] = merged_data_final[['Mean Temperature_x', 'Mean Temperature_ms','Mean Temperature_y']].mean(axis=1)

average_data['Normalized Temperature'] = scaler.fit_transform(average_data[['Mean Temperature']])
...
# Apply moving average
window_size_m = 5  # You can adjust this window size
average_data['Smoothed Temperature'] = moving_average(average_data['Mean Temperature'], window_size_m)
night_data_m['Smoothed Temperature'] = moving_average(night_data_m['Mean Temperature'], window_size_m)

degree_m = 4  # You can change the degree to get a better fit
p_day_ms = Polynomial.fit(average_data.index, average_data['Normalized Temperature'], degree_m)
p_day_all_ms = Polynomial.fit(average_data.index, average_data['Mean Temperature'], degree_m)
p_night_ms = Polynomial.fit(night_data_m.index, night_data_m['Mean Temperature'], degree_m)



# Generate the polynomial values
day_fit = p_day_ms(average_data['Normalized Temperature'].index)
...
ax1.plot(average_data['Date'], day_fit, label='Daytime Temperature for the Biggest Crop Field', color='red', linestyle='--')
